...
class Garden:
	...
	def generate(self):
		...
		self.number_of_yellow_leaves = 0
		self.number_of_orange_leaves = 0
		self.number_of_red_leaves = 0
		self.index = 0
		while True:
			try:
				coordinates = []
				...
				item,y,x = coordinates
				if item == 'R':
					self.garden[y][x] = 'R'
					self.number_of_rocks += 1
				elif item == 'y':
					self.garden[y][x] = 'y'
					self.number_of_yellow_leaves += 1
				elif item == 'o':
					self.garden[y][x] = 'o'
					self.number_of_orange_leaves += 1
				elif item == 'r':
					self.garden[y][x] = 'r'
					self.number_of_red_leaves += 1
	...
	def rake(self,chromosome):
		...
		for gene in range(len(chromosome)):
			...
			start = True
			while running:
				if start == True and (self.garden[y0][x0] == 'R' or (str(self.garden[y0][x0]).isdigit() == True and self.garden[y0][x0] != 0) or (self.garden[y0][x0] == 'o' and self.index != 1 or self.garden[y0][x0] == 'r' and self.index != 2)):
					break
				...
				if self.inside(temp_y0,temp_x0) == False:
					...
					y0 = temp_y0
					x0 = temp_x0
				elif self.garden[temp_y0][temp_x0] == 'y' and self.index == 0 and self.number_of_yellow_leaves != 0:
					self.number_of_yellow_leaves -= 1
					self.garden[y0][x0] = line
					y0 = temp_y0
					x0 = temp_x0
					if self.number_of_yellow_leaves == 0:
						self.index += 1
				elif self.garden[temp_y0][temp_x0] == 'o' and self.index == 1 and self.number_of_orange_leaves != 0:
					self.number_of_orange_leaves -= 1
					self.garden[y0][x0] = line
					y0 = temp_y0
					x0 = temp_x0
					if self.number_of_orange_leaves == 0:
						self.index += 1
				elif self.garden[temp_y0][temp_x0] == 'r' and self.index == 2 and self.number_of_red_leaves != 0:
					self.number_of_red_leaves -= 1
					self.garden[y0][x0] = line
					y0 = temp_y0
					x0 = temp_x0
				else:
					# if y0 == 0 or y0 == self.height-1 or x0 == 0 or x0 == self.width-1 and (self.garden[y0][x0] == 'o' and self.index != 1 or self.garden[y0][x0] == 'r' and self.index != 2):
					# 	# print("NO")
					# 	break
					...
					for move in moves[first_direction]:
						if self.inside(y0+move[0],x0+move[1]):
							if self.garden[y0+move[0]][x0+move[1]] == 0 or (self.garden[y0+move[0]][x0+move[1]] == 'y' and self.index == 0 or self.garden[y0+move[0]][x0+move[1]] == 'o' and self.index == 1 or self.garden[y0+move[0]][x0+move[1]] == 'r' and self.index == 2):
	...
	def get_fitness(self):
		...
		for row in range(self.grdn.height):
			for column in range(self.grdn.width):
				if (str(self.grdn.garden[row][column]).isdigit() and self.grdn.garden[row][column] != 0) or (self.grdn.garden[row][column] == 'R'):
					self.fitness += 1
		return self.fitness


class Evolution:
	def __init__(self,garden,number_of_species,elite,new_blood,cross_percetange,mutation_percetange):
		self.number_of_species = number_of_species
		self.elite = elite
		self.new_blood = new_blood
		self.cross_percetange = cross_percetange
	...
	def start_evolution(self,garden):
		...
		for parent in range(len(parents)):
			for child in range(len(cross[parent])):
				...
				if percentage < self.cross_percetange*100:
...
def test():
	...
	for i in range(100):
		...
		# ae8
		evolution = Evolution(garden,20,0.10,0.10,80,10)
		# evolution = Evolution(garden,20,0.10,0.15,80,7)
